...
class SpeciesRegistry:  # for recording the genotype of different species
    ...
    def get_gene_pool(self, microbiome):  # get a gene pool from a microbiome
        ...
        for i in range(len(microbiome)):
            if microbiome[i] == 0:
                continue
#             gene_0_1 = bin(self.species_list[i][1])[2:]  # bin return ob10010101
#             pad_left = self.number_of_total_genes - len(gene_0_1)  # #calculte the number of 0 at append
#             gene_binary_number = '0' * pad_left + gene_0_1
#             gene_pool += numpy.array([int(k) for k in gene_binary_number]) * microbiome[i]  # [1,0,0,1...]*


#         counts = {}
#         for i in range(len(microbiome)):
#             species_gene_vector = self.species_list[i][1]
#             idx = 0
#             while species_gene_vector:
#                 if species_gene_vector & 1:
#                     try:
#                         counts[idx] += microbiome[i]
#                     except KeyError:
#                         counts[idx] = microbiome[i]
#                 species_gene_vector >> 1
#                 idx += 1
